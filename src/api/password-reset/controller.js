...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Devprince Auth Service account.<br>
        ...
        &mdash; Devprince Auth Service Team
      `
      return sendMail({ toEmail: email, subject: 'Devprince Auth Service - Password Reset', content })
    })
